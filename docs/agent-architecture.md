...
В этом репозитории переносимые MCP-профили не включаются автоматически.
devkit/ добавляет их только по явной команде пользователя в игнорируемый
host-local Codex override; skills остаются переносимыми и не зависят от
конкретной машины. Команды и границы приватности:
